/** Типы продукции — спектр услуг как у поставщика гофротары */
export const productTypes = [
  {
    id: 'four-flap',
    title: 'Четырёхклапанные гофрокороба',
    description:
      'Классические ящики из трёхслойного и пятислойного гофрокартона. Стандартные и индивидуальные размеры.',
    image: '/images/products/four-flap.png',
  },
  {
    id: 'trays',
    title: 'Гофролотки с крышкой',
    description:
      'Лотки для хранения и транспортировки. Удобная сборка, надёжная фиксация крышки.',
    image: '/images/products/trays.png',
  },
  {
    id: 'veg-trays',
    title: 'Овощные лотки',
    description:
      'Гофролотки для овощей и фруктов: вентиляция, штабелирование, удобная выкладка и перевозка с поля и склада.',
    image: '/images/products/veg-trays.png',
  },
  {
    id: 'meat-trays',
    title: 'Лотки под мясную продукцию',
    description:
      'Лотки и тара для мясной продукции: конструкция под вес, штабелирование и логистику охлаждённых и замороженных грузов.',
    image: '/images/products/meat-trays.png',
  },
  {
    id: 'self-lock',
    title: 'Самосборные гофрокороба',
    description:
      'Конструкции без клея и скоб: быстрая сборка на месте, экономия на логистике плоских заготовок.',
    image: '/images/products/self-lock.png',
  },
  {
    id: 'die-cut',
    title: 'Вырубная гофротара',
    description:
      'Сложная высечка и нестандартные конфигурации под продукт: лотки, вкладыши, фигурные короба.',
    image: '/images/products/die-cut.png',
  },
  {
    id: 'microflute',
    title: 'Гофрокартон',
    description:
      'Листы из гофрокартона и микрогофрокартона под любые задачи',
    image: '/images/products/microflute.png',
  },
  {
    id: 'reinforced',
    title: 'Усиленные гофроящики',
    description:
      'Упаковка повышенной прочности для тяжёлых и габаритных грузов. Профили B, E, C, BC, BE.',
    image: '/images/products/reinforced.png',
  },
  {
    id: 'oversized',
    title: 'Крупногабаритная гофротара',
    description:
      'Ящики и конструкции под крупногабаритные грузы: мебель, техника, оборудование. Индивидуальные размеры и усиленные материалы.',
    image: '/images/products/oversized.png',
  },
  {
    id: 'fullcolor',
    title: 'Полноцветная упаковка',
    description:
      'Упаковка с выразительным внешним слоем под брендинг и презентацию продукта.',
    image: '/images/products/fullcolor.png',
  },
  {
    id: 'industry',
    title: 'Отраслевая упаковка',
    description:
      'Решения для пищевой, аграрной, кондитерской, алкогольной, химической промышленности, мебели и техники.',
    image: '/images/products/industry.png',
  },
]

export const fefcoGroups = [
  {
    code: '02',
    title: 'Складные ящики',
    description:
      'Упрощённая конструкция из одной выкройки с клапанами крышки и дна, боковой шов на клей, скобы или ленту.',
  },
  {
    code: '03',
    title: 'Телескопические ящики',
    description: 'Отдельный корпус и крышка; крышка по высоте закрывает корпус.',
  },
  {
    code: '04',
    title: 'Обёрточные и штампованные',
    description:
      'Ящики с язычковыми соединениями без клея, скоб и липких лент.',
  },
  {
    code: '05',
    title: 'Рамные конструкции',
    description:
      'Ящики и детали типа «рама», собираемые скобами, лентой или клеем.',
  },
  {
    code: '06',
    title: 'С обечайкой',
    description:
      'Две торцевые части и соединяющая их обечайка.',
  },
  {
    code: '07',
    title: 'Готовые складные',
    description: 'Собранные в основном из одной детали складные ящики.',
  },
  {
    code: '09',
    title: 'Внутренние элементы',
    description:
      'Прокладки, решётки, вкладыши дна, перегородки и прочие внутренние детали.',
  },
]

/** Материалы и форматы поставки гофрокартона */
export const boardTypes = [
  {
    id: 'three-layer',
    title: 'Трёхслойный гофрокартон',
    description:
      'Два плоских слоя и один гофрированный между ними. Марки Т-21–Т-27, профили B, E, C. Для лёгких и средних грузов, розничной и транспортной упаковки.',
  },
  {
    id: 'five-layer',
    title: 'Пятислойный гофрокартон',
    description:
      'Три плоских слоя и два гофрированных. Марки П-31–П-37, профили BC, BE. Для тяжёлых, габаритных грузов и штабелирования.',
  },
  {
    id: 'z-carton',
    title: 'Z-картон',
    description:
      'Бесконечный гофрокартон, сложенный гармошкой по рилёвке. Трёхслойный (Т-21–Т-27) и пятислойный (П-31–П-37): отрезаете лист нужной длины под габариты груза, меньше отходов.',
  },
]

export const materials = [
  'Трёхслойный гофрокартон марок Т-21–Т-27',
  'Пятислойный гофрокартон марок П-31–П-37',
  'Z-картон (бесконечный) — трёх- и пятислойный',
  'Бурый или белый лицевой слой',
  'Минимальный тираж — по согласованию',
]
